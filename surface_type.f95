module surface_type
  ...
  type, abstract :: surface
     ...
     ! real(kind(1.d0)) :: A=0, B=0, C=0, D=0, E=0, F=0, G=0, H=0, I=0, J=0, L=0, M=0, K=0
   contains
...
contains
  ...
  function create_cylinder1(value1, centered_at) result(cyl)
    type(coordinate), intent(in) :: centered_at
    real(kind(1.d0)), intent(in) :: value1
    class(surface), allocatable :: cyl
    allocate(cylinder :: cyl)
    cyl=cylinder(value1, centered_at)
